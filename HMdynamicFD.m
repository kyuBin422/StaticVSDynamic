function [V,exRegion, M] = HMdynamicFD(X,s0,gamma,kappa,c,F,R,G,b)%,T,n,m)
...
for i = 1:m+2
    ...
    %V(end,i) = g(x(i),s0,gamma,kappa,c,F,R,G,b,s0-10^(-7));
end
...
for nn = n+1:-1:2
    old = B*(V(nn,2:end-1)') + dt*(1-theta)* [ll(1)*V(nn,1); zeros(m-2,1); uu(m)*V(nn,end)]+ dt*(theta)*[ll(1)*V(nn-1,1); zeros(m-2,1); uu(m)*V(nn-1,end)];
